...
public class MessagesController : Controller
{
    ...
    {
        var tmp = await _eventService.GetUserId(GetUserSessionKey());
        if (tmp == sendToUid) return BadRequest("Cannot send a message to yourself.");

        bool success = await _messageService.CreateMessage(message, sendToUid, tmp);
